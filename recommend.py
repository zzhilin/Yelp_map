...
def find_centroid(cluster):
    ...
    x_cord = []
    y_cord = []
    for i in [restaurant_location(restaurant) for restaurant in cluster]:
        x_cord.append(i[0])
        y_cord.append(i[1])
    return [mean(x_cord), mean(y_cord)]
    # END Question 5
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    return max([find_predictor(user, reviewed, feature_fn) for feature_fn in\
                feature_fns], key=lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            reviews_predicted[restaurant_name(restaurant)] =\
            user_rating(user, restaurant_name(restaurant))
        else:
            reviews_predicted[restaurant_name(restaurant)] =\
            predictor(restaurant)
    return reviews_predicted
    # END Question 9
